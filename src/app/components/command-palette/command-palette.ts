...
@Component({
    ...
    <dialog #paletteDialog 
      ...
      <div class="palette-header p-3 border-bottom border-secondary-subtle d-flex align-items-center gap-3">
        ...
        <input #searchInput
               ...
               placeholder="Type a command or search section (e.g. Projects, Theme, Contact)..."
               [(ngModel)]="searchQuery"
               (input)="onSearchInput()"
               (keydown)="onKeydown($event)">
        <span class="badge bg-secondary bg-opacity-25 text-muted font-heading px-2 py-1 me-2 rounded">ESC</span>
      </div>
        ...
        <div *ngFor="let cat of categories">
          ...
          <div *ngFor="let item of getCommandsByCategory(cat); let i = index" 
            ...
            <div class="d-flex align-items-center gap-3">
              ...
              <span class="font-body text-light fw-medium text-truncate" style="max-width: 320px;">{{ item.title }}</span>
            </div>

            <div class="d-flex align-items-center gap-2">
              <span *ngIf="item.shortcut" class="badge bg-dark border border-secondary-subtle text-muted font-heading small px-2 py-1">
                {{ item.shortcut }}
              </span>
  ...
  styles: [`
    .command-palette-dialog {
      ...
      box-shadow: 0 30px 70px rgba(0, 0, 0, 0.8), 0 0 40px rgba(6, 182, 212, 0.25) !important;
    }
...
export class CommandPaletteComponent {
  ...
  searchQuery = '';
  selectedIndex = 0;
  categories: Array<'Navigation' | 'Actions' | 'Theme'> = ['Navigation', 'Actions', 'Theme'];
  ...
  getCommandsByCategory(cat: 'Navigation' | 'Actions' | 'Theme'): CommandItem[] {
